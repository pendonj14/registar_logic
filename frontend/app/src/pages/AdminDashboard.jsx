...
const AdminDashboard = () => {
  ...
  // ... (Keep your existing handlers: handleOpenReview, handleCloseModal, etc. SAME AS BEFORE) ...
  const handleOpenReview = (request) => { setSelectedRequest(request); setIsModalOpen(true); };
  const handleCloseModal = () => { setIsModalOpen(false); setSelectedRequest(null); };
  const handleOpenPayment = (request) => { setSelectedRequest(request); setIsPaymentModalOpen(true); };
  const handleClosePayment = () => { setIsPaymentModalOpen(false); setSelectedRequest(null); };
  const handleOpenRelease = (request) => { setSelectedRequest(request); setIsReleaseModalOpen(true); };
  const handleCloseRelease = () => { setIsReleaseModalOpen(false); setSelectedRequest(null); };
  ...
  return (
    // Cleaned up the container: Removed negative margins and fixed widths
    <div className="min-h-screen bg-[#f8f9fc]">
      ...
      {/* Main Content Area */}
      {/* md:ml-72 adds the margin on desktop so content doesn't go behind sidebar */}
      <main className="flex-1 p-6 md:p-10 md:ml-72 transition-all duration-300">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4">
          ...
          <div className="flex items-center gap-3 md:gap-4 self-end md:self-auto">
                        {/* Mobile Menu Button - Only shows on mobile (md:hidden) */}
            <button 
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          {filteredRequests.length === 0 ? (
            ...
            filteredRequests.map((req) => (
              <div key={req.id} className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100 flex flex-col md:flex-row items-start gap-4 hover:shadow-md transition-shadow">
